...
# ggsave(perc.diff,filename="deluge/figures/DelSize_ANPP.pdf")

# drive_upload(media="deluge/figures/DelSize_ANPP.pdf",path=file.path("deluge","figures"),name="DelSize_ANPP.pdf")
...
gcc.avs<-CCE_gcc%>%
  mutate(new.Date=as.Date(Date,format="%m/%d/%y"))%>%
  group_by(new.Date,treatment)%>%
  dplyr::summarise(
    n.plots=n(),
    gcc.av=mean(GCC_mean),
    gcc.sd=sd(GCC_mean),
    gcc.SE=gcc.sd/sqrt(n.plots))

gcc.qplot.error<-qplot(x=new.Date,y=gcc.av,colour=factor(treatment),group=factor(treatment),data=gcc.avs)+
  geom_point(size=3)+
  geom_line(linewidth=1.5)+
  geom_errorbar(aes(ymin=gcc.av-gcc.SE, ymax=gcc.av+gcc.SE), width=0,
                 position=position_dodge(0.07),size=1.5)

gcc<-gcc.qplot.error+
  labs(title=" ",x="",y="GCC")+
  # scale_x_date(date_labels="%B",
  #              date_breaks  ="1 month",
  #              limits=as.Date(c('2022-05-20','2022-09-20')))+
  # scale_color_manual(values=c("darkgoldenrod3","aquamarine3"),name=c("Treatment"),labels=c("Ambient","Wet"))+
  theme_classic()+
